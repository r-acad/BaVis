...
let CARDS2DATA = {
    ...
    CORDR: function (processed_cards_array){
        ...
        processed_cards_array.forEach((arr_line) =>{
                ...
                let W = myMath.vec_normalize(myMath.makevector_from_points (A, B))  // Unit vector in z direction
                let vec_AC = myMath.makevector_from_points (A, C)  // Vector in plane x-z
                let V = myMath.vec_normalize(myMath.vector_product(W, vec_AC))  // Unit vector in y direction
                let U = myMath.vec_normalize(myMath.vector_product(V, W)) // Unit vector in x direction
...
function get_nastran_fields_from_line (line, is_first_line = false){ // return an array containing the content of the NASTRAN line fields, in short, long and comma separated format
    ...
    if (is_first_line) { return [nfields.field_width,...array_of_fields]  // the first element gives the card format (8 or 16)
